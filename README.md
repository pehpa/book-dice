...
It ships as a command-line tool, a small local web UI, and a native iOS app —
the CLI and web UI share the same `config.json` and selection logic, while
the iOS app is a standalone Swift port of that same logic for when you'd
rather not have a laptop running.
...
A fully native SwiftUI app lives in `ios/` — no server or Python involved at
runtime. `ios/BookDiceKit` is a Swift port of the same pure selection logic
and config model as `core.py`/`config.py`, and `ios/BookDice` is a SwiftUI
front end that walks through the same two-phase flow as the CLI and web UI:
pick a shelf, go gather books, then roll for the winner. Configuration lives
on-device instead of in `config.json`, and instead of the web UI's hard
"weights must sum to 100" requirement, saving a set of categories that don't
add up to 100% just normalizes them automatically.

`ios/BookDice.xcodeproj` is checked into the repo, so open it directly in
Xcode and build/run onto the Simulator or a connected iPhone as usual. The
project file is generated by [XcodeGen](https://github.com/yonaskolb/XcodeGen)
from `ios/project.yml` — if you change that spec (add a file, a target,
etc.), regenerate it:
